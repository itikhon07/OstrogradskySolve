...
// Кэш для предотвращения повторной загрузки
let mathLoadPromise = null;
let physLoadPromise = null;

export async function loadTasksMath() {
    if (tasksMathLoaded) return tasksMath;
...
export async function loadTasksPhys() {
    if (tasksPhysLoaded) return tasksPhys;
    if (physLoadPromise) return physLoadPromise;
    
    physLoadPromise = (async () => {
        try {
            ...
            physLoadPromise = null;
        }
    })();
    
    return physLoadPromise;
}

// Получить нерешенные задачи (исключая решенные)
export function getAvailableTasks(tasks, solvedTaskIds = []) {
    if (!solvedTaskIds || solvedTaskIds.length === 0) return tasks;
    const solvedSet = new Set(solvedTaskIds);
    return tasks.filter(task => !solvedSet.has(task.id));
}